...
Implement a functionality for the teacher to schedule a class.

## 2. Requirements

This US description is "As Teacher, I want to schedule a class".
I need to have courses and teachers created and persisted in the system.

## 3. Analysis

To implement this US, the system asks for the Class information, validates it, it also asks for a Course to link to the class, the students will be automatically added from the Course chosen, it's important to note that we will need to specify a start and close date for the class
since this one is recurrent, when this is done we're going to verify if all the students/teacher are free in the specified date, if they are we're going to create the class,
...
This US consists of some steps, first we need to add a title, then select a course, then select a weekday for the reccurence,
after that we need to select a start and end date, then we need to select a start and end time.
The teacher is automatically added to the class, and the students are added from the course.
...
![a class diagram](CD_1010.svg "Class Diagram")

#### 4.2.2. System Sequence Diagram - SSD

![a class diagram](SSD_1010.svg "System Sequence Diagram Diagram")

#### 4.2.3. Sequence Diagram - SD

![a sequence diagram](SD_1010.svg "Sequence Diagram")
...
To demonstrate this US, we will firs show the main menu.

![1Menu](Support/1.png)

Below we are creating a reccurent class, linked to the course MATCP, we pass all the values and as you can see
we're going to instantiate 3 diffrente classes, one for each week day, before the creation every single class is validated.

![CSV](Support/2.png)

Now, this is that same class on the H2 database

![Console](Support/3.png)

And all the students associated with that class

![Enrollments Repository](Support/4.png)
